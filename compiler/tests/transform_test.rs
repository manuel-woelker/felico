...
pub fn run_transform_test(
    directory: &str,
    transform_fn: impl Fn(&str, &str) -> FelicoResult<String> + Send + 'static + Copy,
) {
    ...
    for file in paths {
        ...
        let testcases = cast!(
            hash.remove(&make_key("testcases")).expect("testcases"),
            YamlElt::Array
        );
        ...
        for testcase in testcases {
            ...
            let name =
                cast!(testcase.remove(name_key).expect("name"), YamlElt::String).replace(" ", "_");
            let input = cast!(testcase.remove(input_key).expect("input"), YamlElt::String);
            let expected_output = cast!(
                testcase.remove(output_key).expect("output"),
                YamlElt::String
            );
            tests.push(Trial::test(format!("{}::{}", prefix, name), move || {
                ...
                assert_eq!(
                    ...
                    "Transformation test failed for '{}' \n\t at {}:{}",
                    name, &filename, marker.line
                );
                Ok(())
            }));
        }
